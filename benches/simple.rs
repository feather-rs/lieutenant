...
use lieutenant::{any, literal, param, Literals, Input, Parser, ParserBase};
...
pub fn single_literal(c: &mut Criterion) {
    ...
    let input: Input = "a9".into();
    c.bench_function("multiple literals or", |b| {
        let input = input.clone();
        b.iter(|| {
            let _ = root.parse(&mut black_box(input));
        })
    ...
    c.bench_function("multiple literals hashmap", |b| {
        let input = input.clone();
        b.iter(|| {
            let _ = root.parse(&mut black_box(input));
        })
